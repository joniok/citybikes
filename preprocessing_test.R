file_list <- list.files(path= "data/json/2018-08", pattern = ".json") # extract from folder "data/json/2018-08" and only files with ".json"
#file_list <- file_list[1:100] # select the first 20 jsons

files_with_path <- paste0("data/json/2018-08/", file_list) # add path to files
...
ldf <- lapply(ldf,function(x){
  ...
  x <- subset.data.frame(x,(x$avl_bikes != x$lag_val) | is.na(x$lag_val)) # remove rows where the number of available bikes is the same between two subsequent rows
  }
  )
